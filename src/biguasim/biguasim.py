...
from biguasim.environments import BiguaSimEnvironment
from biguasim.packagemanager import get_scenario,\
...
def make(scenario_name="", scenario_cfg=None, gl_version=GL_VERSION.OPENGL4, window_res=None, verbose=False,
         show_viewport=True, ticks_per_sec=None, frames_per_sec=None, copy_state=True, start_world=True) -> BiguaSimEnvironment:
    """Creates a BiguaSim environment
    ...
    Returns:
        :class:`~biguasim.environments.BiguaSimEnvironment`: A biguasim environment instantiated
    ...
    if start_world:
        if scenario_name != "":
            ...
            raise BiguaSimException("You must specify scenario_name or scenario_config")
        # Get pre-start steps
    ...
    return BiguaSimEnvironment(**param_dict)
