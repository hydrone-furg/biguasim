...
- ``name`` is a string that specifies the name of the scenario.
   This name is part of the string passed to ``biguasim.make()`` to create an environment.
...
``frames_per_sec`` to true. For running BiguaSim :ref:`headless<headless>`, you'll likely want the 
...
and pass it in to :func:`biguasim.make` using the ``scenario_cfg`` variable.
...
2. Placing it in BiguaSim's scenario search path.

When you give a scenario name to :func:`biguasim.make`, BiguaSim will search 
...
on package folder locations, see :ref:`package-locations`. BiguaSim 
