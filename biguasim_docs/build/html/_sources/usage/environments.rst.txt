...
BiguaSim simulations defined by a scenario are run in an environment object. The environment 
...
BiguaSim environments are created by calling ``biguasim.make()``. The function takes as input 
...
``biguasim.make()`` returns an environment object that you can interact with.

.. note::

    ``biguasim.make()`` is the easiest way to create an environment object and enables the use of
    pre-built scenarios. It builds the binary paths from the provided string and creates a
    ``BiguaSimEnvironment`` object. If you want to create an environment object without using
    pre-built scenarios, you can create a ``BiguaSimEnvironment`` object directly, though this is
    not recommended for most users.

For a full list of inputs to ``biguasim.make()``, see the :ref:`API documentation<biguasim-api-index>`.
