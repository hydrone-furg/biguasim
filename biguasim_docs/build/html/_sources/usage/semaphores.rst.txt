...
=====================================
BiguaSim Client-Server Communication
=====================================
...
In this page we will discuss how the BiguaSim client and engine communicate.
...
===========================
BiguaSim is made up of two halves, the "client" and the "engine".
...
through a simple example. Consider the following code that runs a BiguaSim simulation:
...
The ``biguasim.make()`` function is mostly a helper function to instantiate a
``BiguaSimEnvironment`` object. ``.make()`` loads a configuration file and passes
the appropriate paramaters to the ``__init__()`` of ``BiguaSimEnvironment``.

The ``__init__()`` function does three main things:

1. Starts a process on the engine and tells it the minimum it needs to load
2. Creates BiguaSimClient instance
...
To avoid collisions between different instances of BiguaSim, ``biguasim.make()``
...
At this point of the ``__init__()`` of ``BiguaSimEnvironment``, it creates a 
``BiguaSimClient`` object, which makes two important synchronization semaphores.
These semaphores allow the engine and the client to work in lock-step and 
alternate back and forth (see HolodeckServer.cpp / biguasimclient.py)
...
---------------------
Shared memory buffers are used for a lot in BiguaSim.
...
2. The engine's default UUID is ``""``. This means that if you launch the engine
   ...
   if you specify the UUID is ``""`` when creating the ``BiguaSimEnvironment`` 
   object.